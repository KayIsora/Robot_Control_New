...
void sendPWM(int leftSpeed, int rightSpeed)
{
    ...
    if (file < 0)
    {
        std::cerr << "1 Failed to open the I2C bus." << std::endl;
        return;
    }
    ...
    std::cout << "Left Speed: " << static_cast<int>(buffer[1]) << std::endl;
    std::cout << "Right Speed: " << static_cast<int>(buffer[2]) << std::endl;
    if (write(file, buffer, 3) != 3) // Gửi 2 byte dữ liệu (leftSpeed, rightSpeed)
    {
        std::cerr << "2 Failed to write to the I2C bus." << std::endl;
    }
    close(file);
...
int Distance(int knowWidth, int knowHeight, int frame_width, int frame_height)
{
    ...
    // int Distance = (focalLength * realheight * frame_height) / (knowHeight * sensorheight);
...
void clearFallSignal()
{
    ...
    uint8_t buffer[3];
    buffer[0] = 0x01;// Loại gói tin: báo té ngã
    buffer[1] = 1; // Tín hiệu đặc biệt đại diện cho "té ngã"
    buffer[2] = 1; // Tín hiệu bổ sung
    std::cout << "Left Speed: " << static_cast<int>(buffer[1]) << std::endl;
    std::cout << "Right Speed: " << static_cast<int>(buffer[2]) << std::endl;
    if (write(file, buffer, 3) != 3) {
        std::cerr << "Failed to write fall signal to the I2C bus." << std::endl;
    ...
    close(file);
    std::cout << "clear fall signal" << std::endl;
}
